...
contract('DeedMultiPayouts', (accounts) => {
  ...
  it('Should withdraw for all payouts (1)', async () => {
    for(let i = 0; i < 4; i++) {
      const balanceBefore = web3.utils.toBN(await web3.getBalance(accounts[1]));
      await new Promise(resolve => setTimeout(resolve, 1000));
      deedMultiPayouts.withdraw({from: accounts[0]});
      const balanceAfter = web3.utils.toBN(await web3.getBalance(accounts[1]));
